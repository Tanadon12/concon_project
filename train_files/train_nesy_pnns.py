...
def extract_slot_features(model, inputs):
    """
    Extract features from slot attention model
    Returns flattened features for use in lateral connections
    """
    if hasattr(model.img2state_net, 'get_features'):
        features = model.img2state_net.get_features(inputs)
    else:
        # Get the output from img2state_net directly
        features = model.img2state_net(inputs)
...
def get_model_output(columns, lateral_connections, inputs, task_id):
    """
    Get model output using PNNs architecture with lateral connections
    """
    features = []
            
    # Extract features from all previous columns
    for i in range(task_id):
        with torch.no_grad():  # Previous columns are frozen
            feat = extract_slot_features(columns[i], inputs)
            features.append(feat)
    
    # Extract features from current column
    current_feat = extract_slot_features(columns[task_id], inputs)
    features.append(current_feat)
    
    # Apply lateral connections if not the first task
    if task_id > 0:
        lateral_feats = []
        for i in range(task_id):
        ...
        # Combine current features with lateral features
        combined_feat = torch.cat([current_feat] + lateral_feats, dim=1)
    else:
        combined_feat = current_feat
    
    # Final classification through set classifier
    outputs = columns[task_id].set_cls(combined_feat)
...
def test(columns, lateral_connections, test_loader, current_task_id, device):
    """
    Test function adapted for PNNs architecture
    """
    M_a, M_a_p, M_a_n = [], [], []
...
def train(columns, all_lateral_connections, trainloader, optimizer, criterion, task_id, device=None):
    """
    Training function adapted for PNNs with neuro-symbolic model
    """
    ...
    for data in trainloader:
        ...
        # Get features from all previous columns (frozen)
        for i in range(task_id):
            with torch.no_grad():
                feat = extract_slot_features(columns[i], image)
                features.append(feat)
        
        # Get features from current column
        current_feat = extract_slot_features(columns[task_id], image)
        features.append(current_feat)
...
def create_new_column(args, device):
    """
    Create a new column (model instance) for PNNs
    """
    column = NeSyConceptLearner(
        n_classes=2, 
        n_slots=args.n_slots, 
        n_iters=args.n_iters_slot_att, 
        n_attr=args.n_attr, 
        n_set_heads=args.n_heads, 
        set_transf_hidden=args.set_transf_hidden,
        category_ids=args.category_ids, 
        device=device
    ).to(device)
    
    # Load pretrained slot attention weights
    log = torch.load(args.slot_attention_weights, map_location=torch.device(device))
    column.img2state_net.load_state_dict(log['weights'], strict=True)
...
def run(method, model, train_loaders, val_loaders, test_loaders, test_loaders_global, wandb, args, path, device):
    """
    Main run function adapted for PNNs
    """
    t_a_p, t_a_n, tt_acc = [], [], []

    # Initialize first column by creating a new model instance
    columns = [create_new_column(args, device)]
    
    # Get actual feature dimension from slot attention output
    # We need to do a forward pass to get the correct dimension
    dummy_input = torch.randn(1, 3, 128, 128).to(device)  # Assuming RGB 128x128 input
    with torch.no_grad():
        dummy_features = extract_slot_features(columns[0], dummy_input)
        feature_dim = dummy_features.size(1)
    
    print(f"Detected feature dimension: {feature_dim}")
    
    # Ensure first column has binary output
    columns[0].set_cls = nn.Linear(feature_dim, 2).to(device)
    ...
    for task_id, train_data in enumerate(train_loaders):
        ...
        if task_id > 0:
            ...
            new_column = create_new_column(args, device)
            ...
            new_column.set_cls = nn.Linear(feature_dim * (task_id + 1), 2).to(device)
            columns.append(new_column)
        
        # Set all parameters to non-trainable except current column
        for col_idx, column in enumerate(columns):
            for param in column.parameters():
                param.requires_grad = col_idx == task_id  # Only current column is trainable
        
        # Create optimizer for current column parameters
        trainable_params = []
        
        # Add current column's set_cls parameters (img2state_net is frozen from pretrained)
        trainable_params.extend(list(columns[task_id].set_cls.parameters()))
        ...
        for epoch in range(args.epochs):
            ...
            # Set training mode for current column only
            for i, col in enumerate(columns):
                if i == task_id:
                    col.set_cls.train()
                    col.img2state_net.eval()  # Keep slot attention frozen
                else:
                    col.eval()

            train_epoch_loss, train_epoch_acc, columns, all_lateral_connections = train(
                columns, all_lateral_connections, train_data, optimizer, criterion, task_id, device)
            ...
            eval_dict = {f"train_epoch_loss_t{task_id}": train_epoch_loss, 
                        f"train_epoch_acc_t{task_id}": train_epoch_acc}

            if args.val_flag and task_id in val_loaders:
                valid_epoch_loss_p, valid_epoch_acc_p, valid_epoch_loss_n, valid_epoch_acc_n = validate(
                    columns[task_id], val_loaders[task_id][0], val_loaders[task_id][1], criterion, device)
                
                valid_epoch_loss = (valid_epoch_loss_n + valid_epoch_loss_p)/2
                valid_epoch_acc = (valid_epoch_acc_p + valid_epoch_acc_n)/2

                eval_dict[f"valid_epoch_loss_t{task_id}"] = valid_epoch_loss
                eval_dict[f"valid_epoch_acc_t{task_id}"] = valid_epoch_acc
...
def validate(model, testloader_p, testloader_n, criterion, device):
    """
    Validation function for single model (used during training)
    """
    model.eval()
...
def get_dataset(args):
    """
    Dataset loading function (unchanged from original)
    """
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Continual Coonfounding Dataset - PNNs')
    ...
    parser.add_argument('-m', '--model_name', type=str, default='Slot_Attention', help='select the model you would like to run training with')
    ...
    parser.add_argument("--l2_grads", type=float, default=1, help="Right for right reason weight")

    # Slot attention params
    parser.add_argument('--n-slots', default=10, type=int, help='number of slots for slot attention module')
    parser.add_argument('--n-iters-slot-att', default=3, type=int, help='number of iterations in slot attention module')
    parser.add_argument('--n-attr', default=18, type=int, help='number of attributes per object')

    args = parser.parse_args()

    # Run for multiple seeds like the original PNNs code
    for seed in range(5):
        set_seed(seed)
