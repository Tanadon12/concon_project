...
import wandb
from model import ResNetVAE, ResNet, BasicBlock
from vae import VAE
from utils import *
...
def on_task_update(task_id, trainloader, model, optimizer, criterion, fisher_dict, optpar_dict):
...
def train_ewc(model, shared_model, generator, trainloader, optimizer, generator_optimizer, criterion, task_id, ewc_lambda, fisher_dict, optpar_dict, device, wandb):
    model.train()
    generator.train()
    ...
    for _, (x, y) in enumerate(trainloader):
        ...
        if task_id > 0 :
            with torch.no_grad():
                x_replay = generator.sample(batch_size, device)
                y_replay = model(x_replay).argmax(dim=1)

                # wandb.log({"x_replay_images": [wandb.Image(vutils.make_grid(x_replay[0:10]))]})

            x_combined = torch.cat([x, x_replay], dim=0)
            y_combined = torch.cat([y, y_replay], dim=0)
        ...
        train_running_correct += (preds == y_combined).sum().item()
        
        loss.backward()
        optimizer.step()

        generator_optimizer.zero_grad()
        recon_images, mu, log_var = generator(x)

        # wandb.log({"recon_images": [wandb.Image(vutils.make_grid(recon_images[0:10]))]})
                
        recon_loss = recon_criterion(recon_images, x)
        kl_loss = -0.5 * torch.sum(1 + log_var - mu.pow(2) - log_var.exp()) / batch_size
        
        generator_loss = recon_loss + 0.0001 * kl_loss
    ...
    loss_g = generator_running_loss / counter
    acc = 100. * (train_running_correct / len(trainloader.dataset))
    return loss, reg, loss_g, acc
...
def train_buffer(model, buffer, trainloader, optimizer, criterion, device):
    ...
    # loss and accuracy for the complete epoch
    loss = train_running_loss / counter
    acc = 100. * (train_running_correct / counter)
    return loss, acc
...
def run(method, model, generator, train_loaders, val_loaders, test_loaders, test_loaders_global, wandb, args, device):
    ...
    for idx, train_data in enumerate(train_loaders):
        ...
        for epoch in range(args.epochs):
            ...
            if method == 'ewc_gen':
                train_epoch_loss, reg, loss_g, train_epoch_acc,  = train_ewc(model, model, generator, train_data, optimizer, generator_optimizer, criterion, idx, ewc_lambda, fisher_dict, optpar_dict, device,wandb)
            ...
            eval_dict = {f"train_epoch_loss_t{idx}": train_epoch_loss, f"train_epoch_acc_t{idx}": train_epoch_acc,
                         f"train_generator_t{idx}": loss_g}
        ...
        if method == 'ewc_gen':
            on_task_update(idx, train_data, model, optimizer, criterion, fisher_dict, optpar_dict)
...
if __name__ == '__main__':
    ...
    for seed in range(5):
        ...
        test_acc_p, test_acc_n, test_acc = run(args.method, model, generator, train_loaders, val_loaders, test_loaders, test_loaders_global, wandb, args, device)
